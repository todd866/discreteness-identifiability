...
\textbf{(i) Rank $> 0$:} Suppose $\mathrm{rank}(dh) = 0$ everywhere. Then $dh = 0$, so $h$ is locally constant: $h(x_1) = h(x_2)$ for all $x_1, x_2$ in any connected component. The representation carries no information about the environment state. But then $h(x)$ cannot enable prediction or control---it provides no discrimination between states. This violates Utility. $\Rightarrow$ $\mathrm{rank}(dh) > 0$.

\textbf{(ii) Rank $< \dim X$:} Suppose $\mathrm{rank}(dh) = \dim X$ everywhere, and $\dim \mathcal{S} \geq \dim X$. Then $h$ is an immersion: locally injective, with trivial fibers. Every infinitesimal change in $x$ produces a distinguishable change in $h(x)$.

This violates Stability in three ways:
\begin{itemize}
    \item \textbf{No equivalence classes}: If fibers are singletons, there are no nontrivial equivalence classes---no ``same representation for different microstates.''
    \item \textbf{No noise tolerance}: Any perturbation $x \mapsto x + \delta x$ changes the output. The representation is maximally sensitive to noise.
    \item \textbf{No compression}: The representation has the same complexity as the environment. But then $S$ is not representing $X$; it is \emph{duplicating} $X$. This is not observation but copying.
\end{itemize}

Moreover, Utility requires generalization: the representation must group states that require similar responses. If every state maps to a unique representation, no generalization occurs---the system cannot learn or predict, only memorize.

$\Rightarrow$ $\mathrm{rank}(dh) < \dim X$. \hfill $\square$
