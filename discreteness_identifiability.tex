...
The argument is geometric. Consider a smooth observation map $h: X \to Y$ from a high-dimensional state space $X$ to an observation space $Y$. The differential $dh$ determines which directions in $X$ produce distinguishable outputs in $Y$. The rank of $dh$---the dimension of the identifiable subspace---can only take integer values: $\mathrm{rank}(dh) \in \{0, 1, 2, \ldots, \min(\dim X, \dim Y)\}$.

When parameters or state change the structure of $dh$, the rank can increase or decrease. But it can only jump by integers. Smooth variation in the underlying physics produces discrete transitions in what can be observed.
...
\begin{align}
    [x] := \{x' \in X : h(x') = h(x)\}.
\end{align}
The quotient space $X/{\sim}$ is the space of distinguishable observations. When $h$ is a submersion onto its image (constant rank equal to $\dim h(X)$), the quotient inherits a manifold structure locally diffeomorphic to the image. More generally, the quotient may be a stratified space, with smooth strata corresponding to regions of constant rank.
...
\medskip
\noindent\textbf{Proposition (Capacity Scaling).} Let $h: X \to Y$ be a smooth observation map, and let $U \subseteq X$ be an open region where $\mathrm{rank}(dh) = r$ is constant. Equip $Y$ with a metric. Then:
\begin{enumerate}
    \item[(a)] The image $h(U)$ is locally an $r$-dimensional submanifold of $Y$.
    \item[(b)] If additionally $h(U)$ is bounded with finite $r$-dimensional volume $V_r$, then the $\varepsilon$-covering number satisfies:
    \begin{align}
        N(\varepsilon) \sim C \cdot V_r \cdot \varepsilon^{-r}
    \end{align}
    for small $\varepsilon$, where $C$ depends on the local geometry.
    \item[(c)] Changes in $r$ produce discrete changes in capacity scaling exponents.
\end{enumerate}

\textit{Proof sketch.} Part (a) follows from the \textbf{constant rank theorem}: if $\mathrm{rank}(dh) = r$ on $U$, then locally $h$ is diffeomorphic to the standard projection $\mathbb{R}^n \to \mathbb{R}^r \times \{0\}^{m-r}$. The image is thus locally an $r$-dimensional embedded submanifold.

Part (b) is standard from metric geometry: an $r$-dimensional Riemannian manifold of volume $V_r$ requires $\Theta(\varepsilon^{-r})$ balls of radius $\varepsilon$ to cover. The scaling exponent equals the dimension.

Part (c) follows because the exponent in (b) is exactly $r$, which is integer-valued. A transition from rank $r$ to rank $r+1$ changes the capacity scaling from $O(\varepsilon^{-r})$ to $O(\varepsilon^{-(r+1)})$---a multiplicative factor of $\varepsilon^{-1}$ that diverges as resolution improves. $\square$

\medskip
\noindent\fbox{\parbox{\dimexpr\linewidth-2\fboxsep-2\fboxrule}{%
\textbf{Key assumptions.} The proposition requires: (1) constant rank on $U$ (if rank varies, the image is stratified and the dominant stratum determines scaling); (2) boundedness for finite covering numbers; (3) sufficient regularity (Lipschitz bounds) linking $\varepsilon$-balls in $Y$ to preimages in $X$. These assumptions are satisfied in typical physical applications where phase space is bounded and maps are smooth.}}
\medskip
...
Fisher rank deficiency---where $g_{\kappa\kappa} = 0$ for a coupling parameter $\kappa$---arises under specific structural conditions:

\begin{enumerate}
    \item \textbf{Structural absence}: $\kappa$ is not a parameter in the uncoupled model (different model classes at $\kappa = 0$ vs $\kappa > 0$).
    \item \textbf{Symmetry forcing}: The distribution has a symmetry (e.g., $\kappa \leftrightarrow -\kappa$) that forces $\partial p / \partial \kappa |_{\kappa=0} = 0$.
    \item \textbf{Observation restriction}: The observable channel cannot distinguish $\kappa$-dependent effects below threshold.
\end{enumerate}

\medskip
\noindent\fbox{\parbox{\dimexpr\linewidth-2\fboxsep-2\fboxrule}{%
\textbf{Clarification.} Factorization at $\kappa = 0$ does \emph{not} automatically imply $g_{\kappa\kappa}(0) = 0$. A model can factor at $\kappa = 0$ while still having nonzero first derivative there---the coupling ``turns on'' smoothly. Rank deficiency requires one of the conditions above.}}
\medskip

When coupling activates and rank deficiency is lifted, new directions in parameter space become identifiable. The Fisher rank increases:
\begin{align}
    R(\kappa > 0) > R(0).
\end{align}
...
In quantum mechanics, measurement of observable $\hat{A}$ with spectral decomposition $\hat{A} = \sum_\lambda \lambda P_\lambda$ induces an observation map on density matrices:
\begin{align}
    h(\rho) = \{\mathrm{Tr}(\rho P_\lambda)\}_\lambda
\end{align}
which assigns to each state $\rho$ its \emph{outcome probability distribution}. Two states are observationally equivalent with respect to $\hat{A}$ if they produce the same probability distribution over outcomes:
\begin{align}
    \rho_1 \sim \rho_2 \iff \mathrm{Tr}(\rho_1 P_\lambda) = \mathrm{Tr}(\rho_2 P_\lambda) \text{ for all } \lambda.
\end{align}
This equivalence relation partitions state space; measurement induces a quotient structure.
...
(For continuous spectrum, the spectral decomposition uses projection-valued measures rather than discrete sums, and ``eigenspaces'' are generalized; the quotient formalism extends via spectral measures.)
...
\subsection{A Philosophical Observation (Speculative)}

The theorem suggests---but does not prove---a dissolution of ``why is there something rather than nothing'':

\begin{enumerate}
    \item ``Something'' operationally means ``identifiable structure.''
    \item To ask the question is to observe.
    \item To observe requires $0 < \mathrm{rank}(dh) < \dim X$ (by the theorem).
    \item If $\mathrm{rank}(dh) > 0$, the observer can make distinctions.
    \item Therefore: any observer will find identifiable structure.
\end{enumerate}

The alternative---$\mathrm{rank}(dh) = 0$---is not ``nothing''; it is the absence of observation. This is a \emph{selection effect} argument, not an ontological claim. The theorem shows that observers require dimensional gradients; it does not show that dimensional gradients require observers, nor that ``nothing'' is impossible.
...
\section{Worked Example: Rank Drop on a Singular Locus}

To ground the abstract framework, we present a canonical example where rank changes genuinely within a single smooth map.

\subsection{Setup}

Consider the observation map $h: \mathbb{R}^2 \to \mathbb{R}^2$ defined by:
\begin{align}
    h(x, y) = (x, \, xy).
\end{align}
Here $x$ can be interpreted as a coupling strength and $y$ as an amplitude or internal coordinate.

\subsection{Rank Analysis}

The Jacobian of $h$ is:
\begin{align}
    dh_{(x,y)} = \begin{pmatrix} 1 & 0 \\ y & x \end{pmatrix}.
\end{align}
The determinant is $\det(dh) = x$. Therefore:
\begin{itemize}
    \item $\mathrm{rank}(dh) = 2$ when $x \neq 0$ (generic points)
    \item $\mathrm{rank}(dh) = 1$ when $x = 0$ (the singular locus)
\end{itemize}

This is a \emph{genuine} rank change within a single smooth map. The rank drops discretely---from 2 to 1---on the singular set $\{x = 0\}$.

\subsection{Identifiability Interpretation}

\textbf{Away from the singular locus} ($x \neq 0$): Both coordinates of $h$ vary independently. From observing $(x, xy)$, we can recover $x$ directly and compute $y = (xy)/x$. The full state $(x,y)$ is identifiable; the identifiable dimension equals $\dim X = 2$.

\textbf{On the singular locus} ($x = 0$): The observation is $h(0, y) = (0, 0)$ for all $y$. The entire $y$-axis collapses to a single point. The amplitude $y$ becomes completely unidentifiable; the identifiable dimension drops to 1.

\subsection{The Discrete Transition}

Consider a path crossing the singular locus: $\gamma(t) = (t, 1)$ for $t \in [-1, 1]$.
\begin{itemize}
    \item At $t < 0$: $\mathrm{rank}(dh) = 2$, both coordinates identifiable
    \item At $t = 0$: $\mathrm{rank}(dh) = 1$, amplitude $y$ unidentifiable
    \item At $t > 0$: $\mathrm{rank}(dh) = 2$, both coordinates identifiable
\end{itemize}

The rank function $t \mapsto \mathrm{rank}(dh_{\gamma(t)})$ is integer-valued and lower semicontinuous: it can drop (at $t=0$) but such drops occur on measure-zero sets. Moving away from the singular locus, the rank jumps back up discretely.

\subsection{Physical Interpretation}

This structure appears whenever a coupling parameter ``turns on'' an interaction:
\begin{itemize}
    \item \textbf{Phase transitions}: An order parameter $y$ becomes observable only when symmetry-breaking coupling $x$ is nonzero.
    \item \textbf{Synchronization}: Individual phases become distinguishable only above critical coupling.
    \item \textbf{Measurement}: A quantum amplitude becomes resolvable only when the detector couples to it.
\end{itemize}

The key point: the discrete rank change is not an artifact of changing the parameter space or model class. It is a property of the single map $h$ evaluated at different points in its domain.
